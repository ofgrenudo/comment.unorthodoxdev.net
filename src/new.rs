...
pub async fn comment_on_post(
    ...
    let new_comment_result =
        cmanager::new::comment_on_post(String::from(ip), username, comment_text, post_url.clone());
    debug!("Submitted new comment {:?}", new_comment_result);
    debug!("Redirecting to the root /comment/get/latest");
...
pub async fn post_any_data(
    ...
    let new_comment_result =
        cmanager::new::comment_on_post(String::from(ip), username, comment_text, post_url);
    debug!("Submitted new comment {:?}", new_comment_result);
    debug!("Redirecting to the root /comment/get/latest");
